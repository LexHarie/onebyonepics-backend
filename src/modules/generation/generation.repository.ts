...
export class GenerationRepository implements IGenerationRepository {
  ...
  async findJobsByUserId(userId: string): Promise<GenerationJobRow[]> {
    return this.sql<GenerationJobRow[]>`
  ...
  async findJobsForRecovery(params: {
    ...
    const statusArray = this.sql.array(params.statuses, 'text');
    return this.sql<GenerationJobRow[]>`
      SELECT *
  ...
  async findUploadedImageById(
    uploadedImageId: string,
  ): Promise<UploadedImageRow | null> {
...
export const createGenerationRepository = () =>
  new GenerationRepository(getSql());
