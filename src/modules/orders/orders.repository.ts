...
import type { CreateOrderItemInput, IOrdersRepository } from './domain/orders.repository.interface';
...
export class OrdersRepository implements IOrdersRepository {
  ...
  async insertOrderItems(
    ...
    for (const item of items) {
      const result = await this.sql<OrderItemRow[]>`
  ...
  async markGeneratedImagesPermanent(
  ...
  async findPaidOrderIdsByGenerationJobId(
    generationJobId: string,
  ): Promise<string[]> {
...
export const createOrdersRepository = () => new OrdersRepository(getSql());
