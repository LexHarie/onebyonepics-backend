...
export class AdminOrdersService {
  ...
  async getOrder(orderId: string) {
    ...
    // Generate preview URLs for order items with composed images
  ...
  async generateBulkDownloadZip(
    ...
    if (zipEntries.length === 0) {
      throw new BadRequestException('No composed images available for download');
    }
  ...
  async markAsPrinted(
    ...
    if (!orderRow.admin_downloaded_at) {
      throw new BadRequestException('Order has not been downloaded yet');
    }
